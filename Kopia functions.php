...
class Country
{
    ...
    {
    global $godzin,$minut,$sekund,$db, $country;
    ...
    function MathTime()
    {
        global $godzin,$minut,$sekund,$country,$db,$login_nick,$a_country;
        $a_country = $country['name'];
        if(!empty($country['builid']))
        {
        $ship = $country['builid'];
        if(strstr($ship,"b_")==$ship)
        {
            Country::ModTIme($ship,1,1);
        }
        elseif(strstr($ship,"g_")==$ship)
        {
            Country::ModTIme($ship,2,1);
        }
        $now = gmmktime();
        if($country['builid_t'] <= $now)
        {
            $levela  = $country[$ship]+1;
            $db = mysql_query("UPDATE countries SET $ship='$levela' WHERE owner_id='$login_nick' AND name='$a_country'");
            $db = mysql_query("UPDATE countries SET builid='' WHERE name='$a_country' AND owner_id='$login_nick'");
            $db = mysql_query("UPDATE countries SET builid_t='0' WHERE name='$a_country' AND owner_id='$login_nick'");
            echo("HAHA");
        }
        elseif($now > $country['builid_t'])
        {
            $levela  = $country[$ship]+1;
            $db = mysql_query("UPDATE countries SET $ship='$levela' WHERE owner_id='$login_nick' AND name='$a_country'");
            $db = mysql_query("UPDATE countries SET builid='' WHERE name='$a_country' AND owner_id='$login_nick'");
   ...
   //      wyliczanie calego okresu  //
    ...
    function Builid($type,$level)
    {
        ...
        $a_country = $country['name'];
        $now = gmmktime();
        Country::ModTime($type,$level);
        $btime = gmmktime($godzin,$minut,$sekund,1,0,0);
        $time = $now+$btime;
        ...
        mysql_query("UPDATE countries SET expence='$aa' WHERE owner_id='$login_nick' AND name='$a_country'");
        $mg = mysql_query("UPDATE countries SET builid_t='$time' WHERE owner_id='$login_nick' AND name='$a_country'");
    ...
    function ShowPrice($number_of_price,$builiding,$type,$res)
    {
        ...
        $price2= $first_price*($country[$builiding]+1)*2;
        ...
        else
        {
            switch($type)
            {
                case 1:
                    ...
                    break;
                case 2:
                    return $first_price*($country[$builiding]+1)*2;
                    break;
                case 3:
                    return $first_price*($country[$builiding]+1)*2;
                    break;
    ...
    function GetResources()
    {
        ...
        $rock = $country['rock']+($sekund*(30 * $country['b_km'] * pow(1.1,$country['b_km'])/3600));
        $metal =$country['metal'] +($sekund*(20 * $country['b_kpm'] * pow(1.1,$country['b_kpm'])/3600));
